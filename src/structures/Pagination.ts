...
export class Pagination implements PaginationOptions {
    ...
    public async createSimplePagination(): Promise<Message> {
        ...
        if (this.embeds.length === 1) {
            return this.message.channel.send({ embeds: [this.embeds[0].setFooter({ text: "1 / 1" })] });
        }
        var page = 0;
        ...
        const curPage = await this.message.channel.send({ embeds: [this.embeds[page].setFooter({ text: `${page + 1} / ${this.embeds.length}` })], components: [row] });
        ...
        collector.on("collect", async b => {
            ...
            await curPage.edit({ embeds: [this.embeds[page].setFooter({ text: `${page+1} / ${this.embeds.length}` })] })
        });
